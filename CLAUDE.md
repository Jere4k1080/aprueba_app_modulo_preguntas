...
- Nadie confirmó que el proveedor de correo y contraseña esté habilitado en Firebase Authentication del proyecto `aprueba-app-modulo-preguntas`. Tampoco consta que existan cuentas de demostración. Se crean desde la consola de Firebase, porque el registro de la app no funciona (ADR-41)
...
- ADR-43 a ADR-55 son propuestas de la entrega de Firebase Auth, pendientes de confirmar con el equipo
- Con `FIREBASE_PROJECT_ID=aprueba-dev`, el valor de `backend/.env.example`, el backend local rechaza con 401 los tokens de la app, que vienen del proyecto `aprueba-app-modulo-preguntas`. Falta decidir cómo se prueba la app contra el backend local (ADR-49)
