...
class Correction {
  ...
  factory Correction.fromJson(Map<String, dynamic> j) {
    // El contrato define recompensas como objetos con amount ({amount: int, tier?: string}).
    final rewardGranted = _as<Map>(j['rewardGranted'])?.cast<String, dynamic>();
    final potentialReward = _as<Map>(j['potentialReward'])?.cast<String, dynamic>();
    return Correction(
      id: _str(j['id']),
      ...
      rewardAmount: rewardGranted == null ? null : _int(rewardGranted['amount']),
      potentialReward: potentialReward == null ? null : _int(potentialReward['amount']),
